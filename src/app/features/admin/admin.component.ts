...
export class AdminComponent implements OnInit, OnDestroy {
    ...
    onUsersPageChange(event: PageEvent): void {
        ...
        this.syncPageParam(this.usersPage());
        this.loadUsers();
    }
    ...
    onUsersSort(sort: Sort): void {
        if (!sort.active) {
            ...
            this.usersDirection.set((sort.direction || 'asc') as 'asc' | 'desc');
        }
        this.usersPage.set(0);
    ...
    openUserDetail(user: AdminUserResponse): void {
        ...
        this.adminApi.getApiAdminUsersId(user.id).subscribe({
            next: (u) => {
                ...
                ref.afterClosed().subscribe(() => {
                    this.setQueryParam('userId', null);
                });
            },
            error: () => {
                this.toast.error(ADMIN.failed);
    ...
    private syncPageParam(zeroIndexedPage: number): void {
        this.setQueryParam('page', zeroIndexedPage > 0 ? String(zeroIndexedPage + 1) : null);
    }
    ...
    onSchematicsPageChange(event: PageEvent): void {
        ...
        this.syncPageParam(this.schematicsPage());
        this.loadSchematics();
    }
    ...
    onSchematicsSort(sort: Sort): void {
        if (!sort.active) {
            ...
            this.schematicsDirection.set((sort.direction || 'asc') as 'asc' | 'desc');
        }
        this.schematicsPage.set(0);
        this.syncPageParam(0);
        this.loadSchematics();
    ...
    onStoragePageChange(event: PageEvent): void {
        ...
        this.syncPageParam(this.storagePage());
        this.loadStorage();
    }
    ...
    onStorageSort(sort: Sort): void {
        if (!sort.active) {
            ...
            this.storageDirection.set((sort.direction || 'asc') as 'asc' | 'desc');
        }
        this.storagePage.set(0);
        this.syncPageParam(0);
        this.loadStorage();
    ...
    onTicketsPageChange(event: PageEvent): void {
        ...
        this.syncPageParam(this.ticketsPage());
