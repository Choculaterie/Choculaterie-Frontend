...
export class PublicProfileComponent implements OnInit, OnDestroy {
    ...
    memberYears(p: any): number | null {
        ...
        const diff = Date.now() - new Date(p.registrationDate).getTime();
    ...
    ngOnInit(): void {
        ...
        const enter = (e: Event) => { e.preventDefault(); if (this.isOwnProfile()) this.dragOver.set(true); };
        const over = (e: Event) => e.preventDefault();
        const leave = (e: DragEvent) => { if ((e as DragEvent).relatedTarget === null) this.dragOver.set(false); };
        const drop = (e: Event) => { e.preventDefault(); this.dragOver.set(false); };
        this._dragListeners = { enter, over, leave, drop };
        html.addEventListener('dragenter', enter, true);
        html.addEventListener('dragover', over, true);
        html.addEventListener('dragleave', leave, true);
        html.addEventListener('drop', drop, true);
    ...
    private applyOwnProfile(p: OwnProfileResponse): void {
        ...
        this.ogMeta.setUser({ username: p.username ?? null, biographie: p.biographie ?? null, filePath: p.filePath ?? null });
        this.loading.set(false);
    ...
    submitEmail(): void {
        ...
        this.usersApi.postApiUsersMeRequestUpdate({ email: val.email }).subscribe({
            next: () => { this.editLoading.set(false); this.pendingEmail.set(val.email); this.editStep.set('confirm'); this.scheduleEmailSpamHint(); },
        ...
        this.securityKeysApi.postApiSecurityKeysRegisterOptions({ keyName: this.newKeyName }).subscribe({
            next: async (resp: any) => {
                try {
                    ...
                    if (opts.user?.id) {
                        publicKeyOptions.user = { ...opts.user, id: this.base64UrlToBuffer(opts.user.id) };
                    }
                    if (opts.excludeCredentials?.length) {
                        publicKeyOptions.excludeCredentials = opts.excludeCredentials.map((c: any) => ({
                            ...c, id: this.base64UrlToBuffer(c.id),
                        }));
                    }

                    const credential = await navigator.credentials.create({ publicKey: publicKeyOptions }) as any;
                    ...
                    this.securityKeysApi.postApiSecurityKeysRegister(attestation as any, { sessionId }).subscribe({
                        next: () => {
    ...
    requestPasswordReset(): void {
        ...
        this.passwordResetApi.postApiPasswordResetRequest({ email }).subscribe({
            next: () => { this.resetLoading.set(false); this.resetStep.set('confirm'); this.scheduleResetSpamHint(); },
    ...
    confirmPasswordReset(): void {
        ...
        this.passwordResetApi.postApiPasswordResetConfirm({ email, code: val.code, newPassword: val.newPassword }).subscribe({
            next: () => { this.resetLoading.set(false); this.clearResetSpamHint(); this.resetStep.set('request'); this.resetForm.reset(); this.toast.success(PROFILE.passwordChangedSuccess); },
    ...
    onAvatarDropped(files: File[]): void {
        if (!this.isOwnProfile() || !files.length) return;
        this.handleAvatarFile(files[0]);
    }

    onSplitDrop(e: DragEvent, side: 'avatar' | 'cover'): void {
        e.preventDefault();
        e.stopPropagation();
        this.dragOver.set(false);
        const files = Array.from(e.dataTransfer?.files ?? []);
        if (!files.length) return;
        const imgFiles = files.filter(f => ['image/jpeg', 'image/png', 'image/webp'].includes(f.type));
        if (!imgFiles.length) { this.toast.error('Unsupported format - use JPEG, PNG or WebP'); return; }
        if (side === 'avatar') this.handleAvatarFile(imgFiles[0]);
        else this.handleCoverFile(imgFiles[0]);
    }

    onOverlayLeave(e: DragEvent): void {
        const overlay = e.currentTarget as HTMLElement;
        const related = e.relatedTarget as Node | null;
        if (!related || !overlay.contains(related)) this.dragOver.set(false);
    }
    ...
    onCoverDropped(files: File[]): void {
        if (!this.isOwnProfile() || !files.length) return;
    ...
    ngOnDestroy(): void {
        ...
        if (this._dragListeners) {
            html.removeEventListener('dragenter', this._dragListeners.enter, true);
            html.removeEventListener('dragover', this._dragListeners.over, true);
            html.removeEventListener('dragleave', this._dragListeners.leave, true);
            html.removeEventListener('drop', this._dragListeners.drop, true);
        }
        this.clearMcTimer();
        this.clearDcTimer();
        this.clearEmailSpamHint();
    ...
    private scheduleEmailSpamHint(): void {
        this.clearEmailSpamHint();
        this._emailSpamTimer = setTimeout(() => { this._emailSpamTimer = null; this.toast.showSpamHint(); }, 15_000);
    }

    private clearEmailSpamHint(): void {
        if (this._emailSpamTimer !== null) { clearTimeout(this._emailSpamTimer); this._emailSpamTimer = null; }
        this.toast.dismissSpamHint();
    }

    private scheduleResetSpamHint(): void {
        this.clearResetSpamHint();
        this._resetSpamTimer = setTimeout(() => { this._resetSpamTimer = null; this.toast.showSpamHint(); }, 15_000);
    }

    private clearResetSpamHint(): void {
        if (this._resetSpamTimer !== null) { clearTimeout(this._resetSpamTimer); this._resetSpamTimer = null; }
        this.toast.dismissSpamHint();
    }
