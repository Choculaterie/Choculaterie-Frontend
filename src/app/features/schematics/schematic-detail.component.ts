...
import type { SchematicDetailResponse, SchematicFileResponse, SchematicPictureResponse, AllowedTagResponse, AllowedVersionResponse, AuthorSearchResultResponse } from '../../api/generated.schemas';
...
export class SchematicDetailComponent implements OnInit {
    ...
    readonly filteredVersions = computed(() => {
        ...
        const selected = this.editVersionList();
    ...
    constructor() {
        effect(() => {
            ...
            if (!s || idx >= s.pictures.length) return;
            const filePath = s.pictures[idx].filePath;
            if (!filePath) return;
    ...
    toggleLike(): void {
        ...
        this.schematicsApi.postApiSchematicsIdLike(s.id).subscribe({
            next: (res) => {
                this.schematic.set({ ...s, isLiked: res.isLiked });
    ...
    startEdit(): void {
        ...
        this.editForm.patchValue({
            name: s.name,
            authorName: s.authorExist ?? s.authorName ?? '',
    ...
    onEditPicturesSelected(event: Event): void {
        ...
        this.addEditPictureFiles(newFiles);
        input.value = '';
    ...
    onEditLitematicsSelected(event: Event): void {
        ...
        const input = event.target as HTMLInputElement;
        const newFiles = input.files ? Array.from(input.files) : [];
        this.addEditLitematicFiles(newFiles);
        input.value = '';
        requestAnimationFrame(() => window.scrollTo(0, scrollY));
    }

    onEditFilesDropped(files: File[]): void {
        const images = files.filter(f => f.type.startsWith('image/'));
        const litematics = files.filter(f => f.name.toLowerCase().endsWith('.litematic'));
        if (images.length) this.addEditPictureFiles(images);
        if (litematics.length) this.addEditLitematicFiles(litematics);
    }

    onEditPicturesDropped(files: File[]): void {
        this.addEditPictureFiles(files);
    }

    onEditLitematicsDropped(files: File[]): void {
        this.addEditLitematicFiles(files);
    }
    ...
    onTagInput(q: string): void {
        this.tagInputValue.set(q);
    }

    onVersionInput(q: string): void {
        this.versionInputValue.set(q);
    }

    onEditTagSelected(event: MatAutocompleteSelectedEvent, input: HTMLInputElement): void {
        const val = event.option.value as string;
        if (val === '__suggest__') {
            this.openTagSuggest();
        } else if (this.editTagList().length >= this.MAX_SELECTIONS) {
            this.toast.error('You can select up to 10 tags.');
        } else if (!this.editTagList().includes(val)) {
            this.editTagList.update(tags => [...tags, val]);
        }
        input.value = '';
    ...
    removeEditTag(tag: string): void {
        this.editTagList.update(tags => tags.filter(t => t !== tag));
    }

    onEditVersionSelected(event: MatAutocompleteSelectedEvent, input: HTMLInputElement): void {
        const val = event.option.value as string;
        if (this.editVersionList().length >= this.MAX_SELECTIONS) {
            this.toast.error('You can select up to 10 versions.');
        } else if (!this.editVersionList().includes(val)) {
            this.editVersionList.update(versions => [...versions, val]);
        }
        input.value = '';
    ...
    onEditAuthorSelected(event: MatAutocompleteSelectedEvent): void {
        ...
        this.editForm.controls.authorName.setValue(author.username);
    ...
    removeEditVersion(version: string): void {
        this.editVersionList.update(versions => versions.filter(v => v !== version));
    }

    submitEdit(): void {
        if (this.editForm.invalid || !this.isEditFileValid()) return;
        if (this.editTagList().length > this.MAX_SELECTIONS) {
            this.toast.error('You can select up to 10 tags.');
            return;
        }
        if (this.editVersionList().length > this.MAX_SELECTIONS) {
        ...
        this.schematicsApi.putApiSchematicsId(
            s.id,
            {
                ...
                Tags: this.editTagList().length ? this.editTagList().join(',') : undefined,
                Versions: this.editVersionList().length ? this.editVersionList().join(',') : undefined,
